...
2. `done` — [Claude model-specific weekly metrics](claude-model-weekly-metrics.md) — parse and expose per-model 7-day usage windows (Weekly Sonnet, Weekly Opus) from the Claude API payload.
...
6. `in-progress` — [Consumption color indicators](consumption-color-indicators.md) — color menubar badge and progress bars by the ratio of actual to theoretical consumption across six severity tiers.
